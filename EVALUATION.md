...
## 2026-09-20 Gemini 支持移植

上一条"仍未做的部分"里列的 Gemini 反向 Alpha 还原，至此补齐。

- 新增 `src/gemini-alpha-data.js`：48px / 96px Alpha 模板（来源 WatermarkBatchLite 0.7.2），base64 + zlib 压缩存放；解码后 2,304 + 9,216 = **11,520 bytes**，随代码分发，不额外下载模型。
- 新增 `src/gemini.js`：`detectGemini`（模板匹配 + 候选有效性校验）与 `processGemini`。返回三种状态：
  - `not-found` —— 不认识；
  - `cleaned` —— 反 Alpha 直接解出底色，直接写回画布（**不经过推理**，因此更快）；
  - `needs-inpaint` —— 质量校验不通过，生成异形蒙版退回 LaMa 局部修复（`method: gemini-lama`）。
- `main.js` 的识别段改为**两条线同时跑**：Gemini 独立链路与既有规则引擎并行，任一方命中都不阻断另一方 —— 一张图可能同时带 Gemini 与其他平台的水印。直接还原的结果不计入"待修复区域数"，但水印类型仍计为 Gemini 一处。

验收：24 张 Gemini 测试集，覆盖 `cleaned` 与 `needs-inpaint` 两条分支；并用既有 6 平台样张做回归，确认 Gemini 检测未干扰原有规则（`scripts/browser-gemini-parity.mjs`）。

## 2026-09-20 刷新不白跑：结果持久化

原先只把「所选文件」写进 IndexedDB，Safari 切后台重载后列表虽然回来了，但每张都得重跑。

- IndexedDB 升到 **v2**，新增 `results` store（用 `onupgradeneeded` 补建，老库里的 `images` 不受影响）。
- 每张处理完立即落盘结果；重载后直接还原成「已完成 / 未识别」，**不再重跑**。
- 「未识别 · 保持原图」也落盘 —— 它的结论是"这张不用改"，重跑只会得出同样结论；不恢复反而会让人以为那张没处理过。
- 落盘前按**实时余量**判断（不是静态阈值），不够就在队列行说明原因，不静默丢。

验收：真实刷新后已完成的条目直接可用；`未识别` 与 `失败` 两种状态的恢复行为分别断言。

## 2026-09-20 推理搬进独立 Worker（v0.12.0）

主线程上跑 `session.run` 会把界面冻住，而且超时只能"不再等待"、停不掉底层推理。改为把 ONNX 放进独立 Web Worker。

| 指标（20 张 Gemini，同机同批，模型已缓存） | 主线程推理（v0.11.2） | Worker 推理（v0.12.0） |
| --- | ---: | ---: |
...
- `src/inference-worker.js`（`?worker&inline`，内联避免 Pages 上的二次脚本请求）+ `src/inference-controller.js`：两个文件都**逐字节复制**自参考版，SHA-256 自证零改写。
- 超时改为 `worker.terminate()`：进程级终止，WASM 内存与线程池一并归还，比 `session.release()` 可靠。
- OOM 免刷新重建：当场 terminate 旧 Worker、以 2/1 线程重建、重试同一张。

### 反直觉发现：最长长任务几乎没变

单次最长长任务 5546 ms → 4685 ms，改善甚微。只看这一项会误判"移植无效"。
CDP Profiler 归因（先 `vite build --minify false` 保留函数名）：

| 热点 | 占主线程 | 定义于 |
| --- | ---: | --- |
| `ccorrMax` | 35.3% | `imaging.js`（由 `rules.js` 调用） |
| `nccMax` | 13.1% | `imaging.js`（由 `rules.js` 调用） |

那是水印定位的互相关搜索，合计约 48%，与推理无关，也不在本次改动范围。**要看平均与累计，不能被最大值误导**；界面上「页面短暂无响应属于正常现象」因此依然成立。

## 2026-09-21 线程降级策略：先做错一版，再改对（v0.13.0 → v0.14.0）

小内存机型（如 iPhone 13）在多线程下会撞 ORT WASM 分配失败。怎么记住"这台设备吃不消"，我做过两版：

**v0.13.0（我的第一版）**：把降级写进 `localStorage`，带 **24 小时有效期**，到期自动恢复；旧格式就地迁移。附 18 项单测。

**v0.14.0（改对）**：降级**只写当前页面的内存变量**，刷新或重开即按设备能力重新评估；启动时 `removeItem` 清掉旧版遗留的持久值。策略抽到 `src/thread-policy.js`（**逐字节 cp 自参考版 Codex v0.4.2**，SHA-256 `2ecef278…`）。

为什么第二版更好——不是"更短"，是**行为上更对**：

1. OOM 的触发原因通常是**动态**的（当时后台开着别的标签页）。内存压力一缓解，用户刷新本该立刻回到全速，而 TTL 方案会让他白白慢满 24 小时。
2. **刷新本身就是一次免费的重新探测**，成本（最坏多花几秒重建一次会话）远小于"被错误限制一整天"。
3. 24 小时是**拍出来的参数**，没有依据。
4. 少一份隐藏状态就少一类 bug（格式迁移、清除旧值、隐私模式写入失败、用户困惑"为何一直很慢"）。

顺带从参考版学回两处验证手法，都已落地：

- **把架构约束写成可执行断言**：`scripts/verify-thread-policy.mjs` 直接读 `src/main.js` 源码，断言「不许对这个键 `getItem`/`setItem`」+「必须有 `removeItem`」。注释拦不住后来的人，断言可以。
- **UI 断言先种旧值再 reload**：`browser-ui-check.mjs` 先 `localStorage.setItem('lama-threads','1')`，reload 后断言它已被清除 —— 单测只能证明源码里有那行代码，只有真跑一次才能证明它在页面上生效。

验收：策略单测 15 项 + 2 项源码约束；真实 WebKit 四场景（干净态 / 旧残留 `"1"` / 旧残留 `"2"` / 运行期才塞入的值）全部符合预期。

### 本轮踩到的两个环境坑

1. **对线上地址跑断言时不能假设"连上就能操作页面"**：CDP 连上目标页的那一刻导航可能还没完成，`location.origin` 还是 `null`，访问 `localStorage` 会抛 `SecurityError`。本地服务响应快所以永远复现不了，线上首屏慢才暴露。先轮询 `document.readyState === 'complete' && location.origin !== 'null'`。
2. **比对两个独立构建不能直接比 hash**：vite 会把构建时间戳注入 bundle，同一份源码构建两次字节数可相同、SHA-256 必然不同。要先剔除时间戳（及随构建日变化的日期）再比剩余内容。同理，测试断言里不许写死构建日期 —— 那会在第二天变成假红。
