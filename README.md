...
- **超时能真正终止**：旧的 `Promise.race([run, timeout])` 只是"不再等待"，底层 `session.run` 仍在跑、WASM 内存也不释放。现在超时直接 `worker.terminate()`——进程级终止，连带回收 WASM 内存与线程池。
- **OOM 免刷新重建**：内存不足时当场 `terminate()` 旧 Worker，以 2 线程（必要时 1 线程）重建会话并**重试同一张**。旧版必须刷新页面才生效。
- **线程数跟随设备，不做持久化**：跨源隔离生效时最多 4 线程，否则退回 1 线程。OOM 后按 4 → 2 → 1 降级；降级**只记在本次页面的内存里**，刷新或重开就按设备能力重新评估——一次偶发 OOM（例如当时后台开着别的标签页）不会把设备永久钉在低线程。旧版曾把降级写进 `localStorage`，升级后会在启动时主动清除。
